import * as React from "react";
import Toast from "react-native-simple-toast";
import * as firebase from "firebase";
import dao from "../firebase/dao";

const AuthContext = React.createContext();

function AuthProvider(props) {
  const [isSignedIn, setSignedIn] = React.useState(false);
  const [user, setUser] = React.useState(null);
  const [teams, setTeams] = React.useState([]);
  const [notification, setNotifications] = React.useState([]);

  const [unsubNotifications, setUnsubNotifications] = React.useState(null);

  const auth = firebase.auth();
  const db = firebase.firestore();

  React.useEffect(() => {
    signOut();
    signIn({ email: "arthur.olmos@gmail.com", password: "123456" });

    auth.onAuthStateChanged(async (user) => {
      console.log("On Auth State changed...");

      if (user) {
        try {
          const { uid } = user;
          const userRef = db.collection("users").doc(uid);

          await getTeams(uid);

          //Add realtime listener on Notifications
          const unsubNotifications = userRef
            .collection("notifications")
            .onSnapshot((snapshot) => {
              const notifications = [];

              snapshot.forEach((doc) => {
                const notification = doc.data();
                notification.id = notification.id;

                notifications.push(notification);
              });

              setNotifications([...notification]);
            });
          // setUnsubNotifications(unsubNotifications);
          // });

          setUser({ ...user });
          setSignedIn(true);
        } catch (err) {
          console.log("ERROR ON SETTING UP", err);
        }
      } else {
        if (unsubNotifications) unsubNotifications();
        setSignedIn(false);
        setUser(null);
        setTeams([]);
      }
    });
  }, []);

  function refreshUser() {
    const user = firebase.auth().currentUser;
    setUser({ ...user });
  }

  async function getTeams(uid = user.uid) {
    console.log("Getting teams...");

    try {
      const userTeamsRef = db.collection("users").doc(uid).collection("teams");

      const userTeams = await dao.getCollection(userTeamsRef);
      const teamsId = userTeams.map((team) => team.id);
      if (teamsId.length > 0) {
        const teamsRef = db
          .collection("teams")
          .where(firebase.firestore.FieldPath.documentId(), "in", teamsId);
        const teams = await dao.getCollection(teamsRef);

        setTeams([...teams]);
      } else {
        setTeams([]);
      }
    } catch (err) {
      console.log("ERROR ON LOADING TEAMS", err);
    }
  }

  async function signIn({ email, password }) {
    console.log("Signing in...");

    await firebase
      .auth()
      .signInWithEmailAndPassword(email, password)
      .catch((err) => {
        console.log("ERROR ON SIGN IN ", err);
        Toast.show(err.message);
      });
  }

  async function signUp({ email, password, firstName, lastName }) {
    console.log("Signing up...");

    // firstName = "Hello";
    // lastName = "World";
    // password = "123456";

    if (
      email !== "" &&
      password !== "" &&
      firstName !== "" &&
      lastName !== ""
    ) {
      return auth
        .createUserWithEmailAndPassword(email, password)
        .then(async (result) => {
          const { user } = result;

          return user
            .updateProfile({
              displayName: firstName + " " + lastName,
            })
            .then(async () => {
              refreshUser();

              const userRef = db.collection("users").doc(user.uid);

              await dao.setDocumentWithId(userRef, {
                email,
                firstName,
                lastName,
              });

              // return userRef
              //   .set({
              //     email,
              //     firstName,
              //     lastName,
              //     _createdAt: firebase.firestore.FieldValue.serverTimestamp(),
              //     _updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
              //   })
              //   .catch((err) => {
              //     console.log("ERROR ON SAVING USER IN DB - SIGNUP", err);
              //   });
            });
        })
        .catch((err) => {
          console.log("ERROR ON CREATING USER - SIGNUP", err);
          Toast.show(err.message);
        });
    } else {
      Toast.show("Preencha os campos corretamente!");
    }
  }

  async function signOut() {
    console.log("Signing out...");

    await auth.signOut().catch((err) => {
      console.log("ERROR ON SIGNOUT", err);
      Toast.show("Erro ao sair!");
    });
  }

  return (
    <AuthContext.Provider
      value={{
        signIn,
        signOut,
        signUp,

        isSignedIn,
        setSignedIn,

        user,
        setUser,

        teams,
        setTeams,

        refreshUser,
        refreshTeams: getTeams,
      }}
    >
      {props.children}
    </AuthContext.Provider>
  );
}

export { AuthContext, AuthProvider };
